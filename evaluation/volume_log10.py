...
if __name__ == '__main__':
    ...
    while order <= settings.ITERATOR_NUMBER:
        ...
        y_values1.append(math.log10(iterator_values1[order]))
        order += 1
    ...
    while order <= settings.ITERATOR_NUMBER:
        x_values.append(order)
        y_values2.append(math.log10(iterator_values2[order]))
        order += 1
    ...
    while order <= settings.ITERATOR_NUMBER:
        x_values.append(order)
        y_values3.append(math.log10(iterator_values3[order]))
        order += 1

    label = ""+str(((settings.TIME_SLICE*user_num*poi_num)**2))
    pylab.plot(x_values, y_values3, 'gs', linewidth=1, linestyle="-", label=label)
